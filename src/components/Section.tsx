...
const SectionTitle: FC<ComponentProps<typeof motion.span>> = ({
  ...
  children ? (
    <motion.span
      ...
      viewport={{ once: true }}
      className={cn(
        "text-primary w-fit px-6 py-2 rounded-full text-base font-medium text-center mb-10 inline-block",
        className
      )}
      {...props}
    >
...
const SectionSubTitle: FC<ComponentProps<typeof motion.span>> = ({
  ...
  children ? (
    <motion.span
      ...
      viewport={{ once: true }}
      className={cn(
        "px-4 text-default-800 sm:text-xl text-base sm:leading-10 leading-9 font-medium text-center mt-4 mb-6 inline-block",
        className
      )}
      {...props}
    >
